...
struct KeyData LogitechG815::getInput() {
	...
	if (nBytes == 8 && buf[0] == 0x11 && buf[1] == 0xff && buf[3] == 0x00) {
		 ...
		 *
		 * G1	0x11 0xff 0x0a 0x00 0x01 - buf[4]
		 * G2	0x11 0xff 0x0a 0x00 0x02 - buf[4]
		 * G3	0x11 0xff 0x0a 0x00 0x04 - buf[4]
		 * G4	0x11 0xff 0x0a 0x00 0x08 - buf[4]
		 * G5	0x11 0xff 0x0a 0x00 0x10 - buf[4]
...
void LogitechG815::handleKey(struct KeyData *keyData) {
	if (keyData->index != 0) {
		...
		if (keyData->type == KeyData::KeyType::Macro) {
			...
			} else if (keyData->index == G815_KEY_MR) {
				/* MR key */
